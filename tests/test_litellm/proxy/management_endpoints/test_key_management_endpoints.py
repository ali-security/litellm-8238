...
class TestPassthroughRoutesCallerPermission:
    """
    Non-admins must not be able to set ``allowed_passthrough_routes`` on a key.
    """

    def test_non_admin_passthrough_routes_blocked(self):
        from litellm.proxy.management_endpoints.key_management_endpoints import (
        ...
        with pytest.raises(HTTPException) as exc_info:
            _check_passthrough_routes_caller_permission(data, user)
        assert exc_info.value.status_code == 403

    def test_non_admin_passthrough_routes_in_metadata_blocked(self):
        from litellm.proxy.management_endpoints.key_management_endpoints import (
            _check_passthrough_routes_caller_permission,
        )
        data = GenerateKeyRequest(
    ...
    def test_admin_passthrough_routes_allowed(self):
        from litellm.proxy.management_endpoints.key_management_endpoints import (
            _check_passthrough_routes_caller_permission,
        )
        data = GenerateKeyRequest(
    ...
    def test_non_admin_no_passthrough_routes_ok(self):
        from litellm.proxy.management_endpoints.key_management_endpoints import (
            _check_passthrough_routes_caller_permission,
        )
        data = GenerateKeyRequest(key_alias="plain-key")
